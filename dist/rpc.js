...
import { CONFIG_DIR, VENV_DIR } from "./config.js";
...
function toKind(kind) {
    const valid = ['session_expired', 'rate_limited', 'forbidden', 'not_found', 'auth', 'not_supported', 'upstream'];
    return valid.includes(kind) ? kind : 'upstream';
}
/** Locate or create the Python venv with twifork installed. */
export async function ensureVenv(onLog, force = false) {
    ...
    try {
        ...
        throw new Error(`Python setup failed and uv was not found. Install Python 3.10+ or uv (https://docs.astral.sh/uv/). Detail: ${e.message}`);
    }
...
export class Bridge extends EventEmitter {
    ...
    async start(timeoutMs = 120_000) {
        ...
        proc.on('exit', (code) => {
            ...
            if (this.starting)
                return; // initial start failed; do not auto-restart here
            this.restarts += 1;
        ...
        while ((idx = this.buffer.indexOf('\n')) >= 0) {
            ...
            if (id !== undefined && id !== null && (msg['result'] !== undefined || msg['error'] !== undefined)) {
                ...
                if (msg['error'] !== undefined) {
                    const err = msg['error'];
                    pending.reject(new RpcError(err.code ?? -32000, err.message ?? 'unknown error', toKind(err.data?.kind), (err.data ?? {})));
                }
                else {
