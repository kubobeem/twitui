...
import { decodeImage, fetchImage, resize, fitCells, type RawImage } from './decode.ts';
...
export function renderBraille(img: RawImage, maxCols = 60, maxRows = 30, threshold = 128): string {
...
export function kittyPngEscape(png: Buffer, imageId: number): string {
...
export async function renderImageFromUrl(url: string, maxCols = 60, maxRows = 30): Promise<string | null> {
